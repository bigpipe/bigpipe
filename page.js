...
_.extend(Page.prototype, EventEmitter2.prototype, {
  ...
  , compose: function compose() {
      ...
      // Generate an id from the files to see if it's already cached before.
      id = Object.keys(assets).sort().join();
      cached = cache.get(id);
  ...
  , receive: function dispatch(pagelet, event, args) {
      if (!(pagelet in this.pagelets)) return false;
      var receiver = this.pagelets[pagelet];

      // Emit the event
      args.unshift(event);
      return receiver.emit.apply(receiver, args);
    }
     ...
     * @param {String} event name of the event that should be triggered
     * @param {Array} args the event arguments
     * @returns {Boolean} successfull transmisstion to each connection
     * @api public
     */
  , trigger: function trigger(pagelet, event, args) {
    ...
    /**
     * Iterate over all the pagelets and do something with them.
     *
     * @param {Function} cb called for each pagelet
     */
  , forEach: function forEach(cb) {
      var self = this
        , pagelets = this.pagelets;

      Object.keys(pagelets).forEach(function pagelets(name, index) {
        var pagelet = pagelets[name];

        cb.call(pagelet, pagelet, index, pagelets);
      });
    }

    /**
     * Destroy the page and all it's references.
     */
  , destroy: function destroy() {
      var self = this;

      // Kill all the pagelets that are attached to this page.
      this.forEach(function eachPagelet(pagelet) {
        pagelet.destroy();
      });
