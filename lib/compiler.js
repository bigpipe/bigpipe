...
Compiler.prototype.namespace = function prefix(file, fn) {
  ...
  //
  if ('.css' !== path.extname(file.pathname)) return fn(null, file);
...
Compiler.prototype.register = function register(traits) {
  var alias = Array.isArray(traits.alias) ? traits.alias : [ traits.alias ]
    , pathname = traits.pathname
  ...
  traits.code = this.buffer[pathname] = new File(code, extname);

  //
  // Add file references to alias and origin by file and dirname from alias,
  // unique in 99.9% of the cases.
  //
  alias.forEach(function add(alias) {
    var simple = path.sep + alias.split(path.sep).splice(-2).join(path.sep);

    compiler.origin[traits.origin || alias] = simple;
    compiler.alias[simple] = pathname;
  });

  this.emit('register', traits);
  return this.save(filename, traits.code);
};
...
Compiler.prototype.catalog = function catalog(pages, done) {
  ...
  pages.forEach(function each(Page) {
    ...
    page.pagelets.forEach(function each(Pagelet) {
       ...
       */
      function add(file) {
        if (/^(http:|https:)?\/\//.test(file)) return;
