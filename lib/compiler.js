...
Compiler.prototype.catalog = function catalog(pages, done) {
  ...
  async.waterfall([
    ...
    compiler.bigPipe(function browserified(err, buffer) {
      ...
      //
      // Also register the file under the name of the fittings so it can also be
      // laoded _without_ knowing the md5.
      //
      compiler.buffer['/'+ compiler.pipe._framework.get('name') +'.js'] = file;
