...
Compiler.prototype.bigPipe = function bigPipe(client) {
  ...
  return this.register(
    '/dist/pipe.js',
    this.pathname + name + '.js',
    library,
    path.resolve(__dirname, '../dist/pipe.js')
  );
};
 ...
 * @param {String} origin optional The original pathname before compiling
 * @api private
 */
Compiler.prototype.register = function register(alias, pathname, code, origin) {
...
Compiler.prototype.catalog = function catalog(pages, done) {
  ...
  function prefab(filepath, fn) {
    ...
    compiler.process(filepath, function store(error, content) {
      ...
      compiler.register(
        filepath,
        compiler.destination(filepath, content),
        content
      );
