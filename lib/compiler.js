...
function Compiler(directory, pipe, options) {
  ...
  this.client = fs.readFileSync(path.resolve(__dirname, '../dist/pipe.js'), 'utf-8');
...
Compiler.prototype.bigPipe = function bigPipe(client) {
  var library = [
    ...
    '// https://github.com/3rd-Eden/bigpipe',
    '//',
...
Compiler.prototype.page = function find(page, assets) {
  var css = page.dependencies['.css'] || []
    , js = page.dependencies['.js'] || []
    , compiler = this;
  ...
  if ('sync' === page.mode) page.enabled.forEach(function enabled(pagelet) {
    Array.prototype.push.apply(css, compiler.pagelet(pagelet).css);
  });

  (css).concat(js).forEach(function each(dependency) {
    assets.push(compiler.html(compiler.resolve(dependency)));
  });
