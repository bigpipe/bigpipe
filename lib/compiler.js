 ...
 * @param {String} hash sha1 representation of the parsed content
 * @api public
 */
Compiler.prototype.destination = function destination(filepath, hash) {
...
Compiler.prototype.put = function put(filepath) {
  ...
  compiler.process(filepath, function processed(error, code) {
    ...
    hash = compiler.hash(code);
    compiler.emit('preprocessed', filepath);

    compiler.register({
      pathname: compiler.destination(filepath, hash),
      alias: filepath,
      code: code,
      hash: hash
    });
 ...
 *   - hash {String} sha1 hash of the orginal content
 *
 * @param {Object} representation of parsed content.
 * @param {Function} fn completion callback.
 * @api public
 */
Compiler.prototype.namespace = function prefix(file, fn) {
  //
  // Only prefix if the code is CSS content.
  //
  if ('.css' !== path.extname(file.pathname)) return fn();
...
Compiler.prototype.catalog = function catalog(pages, done) {
  ...
  /**
   * Filter out external dependencies, like jQuery and similar. Also remove them
   * from the list so it can be reused later on.
   *
   * @param {String} filepath absolute path to file.
   * @return {Boolean} keep or remove dependency.
   * @api private
   */
  function external(filepath) {
    if (!/^(http:|https:)?\/\//.test(filepath)) return true;

    delete assemble[filepath];
    return false;
  }
  ...
  function prefab(filepath, fn) {
    compiler.process(filepath, function store(error, code) {
      if (error) return fn(error);
      var hash = compiler.hash(code);

      list[hash] = compiler.merge(list[hash] || {}, {
        pathname: compiler.destination(filepath, hash),
        pagelets: assemble[filepath],
        alias: filepath,
        code: code,
        hash: hash
      });

      assemble[filepath] = hash;
      fn(null, code);
    });
  }

  function namespace(hash, fn) {
    compiler.namespace(list[hash], fn);
  }
  ...
  pages.forEach(function each(Page) {
    ...
    page.pagelets.forEach(function each(Pagelet) {
      ...
      function add(file) {
        file = assemble[file] = assemble[file] || [];
        if (!~file.indexOf(pagelet.name)) file.push(pagelet.name);
      }

      if (pagelet.js) add(pagelet.js);
      if (pagelet.css) add(pagelet.css);
  ...
  //
  async.each(Object.keys(assemble).filter(external), prefab, function register(error) {
    if (error) return done(error);

    async.each(Object.keys(list), namespace, function namespaced(error) {
      if (error) return done(error);

      Object.keys(assemble).forEach(function add(file) {
        file = assemble[file];
        compiler.register(list[file]);
      });
