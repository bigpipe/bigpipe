...
Compiler.prototype.catalog = function catalog(pages, done) {
  ...
  pages.forEach(function each(Page) {
    ...
    page.pagelets.forEach(function each(Pagelet) {
      ...
      if (!view.hash) view.hash = { client: crypto.createHash('md5').update(view.client).digest('hex') };
      compiler.register(new File(pagelet.view, '.js', false, 'pipe.templates["'+ view.hash.client +'"] ='+ view.client +';'));
    });
