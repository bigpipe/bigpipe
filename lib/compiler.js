...
Compiler.prototype.catalog = function catalog(pages, done) {
   ...
   * from the assemble so it can be reused later on.
   *
  ...
  function prefab(filepath, fn) {
    compiler.process(filepath, function store(error, code) {
      ...
      assemble[hash] = compiler.merge(assemble[hash] || {}, {
        pathname: compiler.destination(filepath, hash),
        pagelets: list[filepath],
        alias: filepath,
        code: code,
        hash: hash
      });
  ...
  function namespace(hash, fn) {
    compiler.namespace(assemble[hash], fn);
  }
  ...
  //
  // list the CSS/JS of the pagelet with prefab.
  //
  async.each(Object.keys(list).filter(external), prefab, function register(error) {
    if (error) return done(error);

    async.each(Object.keys(assemble), namespace, function namespaced(error) {
      if (error) return done(error);

      Object.keys(list).forEach(function add(file) {
        file = list[file];
        compiler.register(assemble[file]);
      });
