 ...
 * @api private
 */
Compiler.prototype.put = function put(filepath, fn) {
  var compiler = this
    , dest;
  ...
  compiler.process(filepath, function processed(error, code) {
    ...
    compiler.register(filepath, dest, code);
    fn(error, dest);
  });
};
  ...
  function prefab(filepath, fn) {
    ...
    compiler.process(filepath, function store(error, content) {
      ...
      var filename = compiler.hash(content)
        , dest = compiler.destination(filepath, content);
      ...
      //
      if (~dest.indexOf('.css')) {
        content = content + '#pagelet_'+ filename + ' { height: 42px }';
      }

      compiler.register(filepath, dest, content);
      fn(error, content);
    });
  }
