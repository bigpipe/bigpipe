...
//
var pipejs = path.join(require.resolve('bigpipe.js'), '../dist/pipe.js')
...
Compiler.prototype.bigPipe = function bigPipe(client) {
  ...
  for (name in this.pipe._plugins) {
    ...
    library.push(
      'this["BigPipe"].prototype.plugins['+ name +'] = '+ plugin.client.toString() +';'
    );
  }

  debug('creating the bigpipe.js front-end library which contains the client information and core');
  return this.register(new File(local, '.js', true, library.join('\n')));
};
...
Compiler.prototype.catalog = function catalog(pages, done) {
  ...
  pages.forEach(function each(Page) {
     ...
     */
    function view(path) {
      debug('Attempting to compile the view  '+ path);

      var data = temper.fetch(path);
      ...
      compiler.register(new File(path, '.js', false, 'pipe.templates["'+ data.hash.client +'"] ='+ data.client +';'));
    }
    ...
    //
    page._dependencies = dependencies.concat(local).reduce(function reduce(memo, dependency) {
  ...
  async.waterfall([
    ...
    if (!err) compiler.bigPipe(compiler.client);
    done(err, data);
  });
};
